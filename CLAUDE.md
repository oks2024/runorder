...
| Local save / export-import | `src/io/`, `src/store/libraryStore.ts` | autosave (`persist` on the live spec, key `prewire.live`, corrupt-blob fallback to seed), named library (key `prewire.library`; identity = `spec.name`; `isDirty` unsaved-work check), JSON export/import (`{ prewire, spec }` envelope, two-stage validate); UI = `LibraryMenu` "Saved" dropdown in TopBar (New/Open/Save/Save as/Export/Import/Manage, discard + collision confirms). One-way per guardrail #1 — files carry the spec, never emitted scripts |

Test suite: **187 passing** (schema, store, validation, memory names, both emitter snapshots + emit-lines provenance, rehearse derivation, persistence/library/file-IO, RTL UI incl. rehearsal + receipt hover + library menu). `npm run typecheck` / `build` clean (`lint` is red only on the untracked `test/Short story/script.js` fixture).
