...
Test suite: **215 passing** (schema, store, validation, memory names, both emitter snapshots + emit-lines provenance, rehearse derivation, persistence/library/file-IO incl. the `prewire.*`→`playsheet.*` storage-key migration, RTL UI incl. rehearsal + receipt hover + library menu). `npm run typecheck` / `build` clean (`lint` is red only on the untracked `test/Short story/script.js` fixture).
