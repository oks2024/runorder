...
| Local save / export-import | `src/io/`, `src/store/libraryStore.ts` | autosave (`persist` on the live spec, key `playsheet.live`, corrupt-blob fallback to seed), named library (key `playsheet.library`; identity = `spec.name`; `isDirty` unsaved-work check), JSON export/import (`{ playsheet, spec }` envelope, two-stage validate; legacy `{ prewire, spec }` still imports), one-time localStorage key migration from the `prewire.*` keys (`src/io/storage.ts`); UI = `LibraryMenu` "Saved" dropdown in TopBar (New/Open/Save/Save as/Export/Import/Manage, discard + collision confirms). One-way per guardrail #1 — files carry the spec, never emitted scripts |

Test suite: **215 passing** (schema, store, validation, memory names, both emitter snapshots + emit-lines provenance, rehearse derivation, persistence/library/file-IO incl. the `prewire.*`→`playsheet.*` storage-key migration, RTL UI incl. rehearsal + receipt hover + library menu). `npm run typecheck` / `build` clean (`lint` is red only on the `test/Short story/script.js` fixture — a captured runtime workflow script, not app code).
