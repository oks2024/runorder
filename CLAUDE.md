...
| State | `src/store/workflowStore.ts`, `src/store/uiStore.ts` | Zustand + Immer; the single live `WorkflowSpec`; one generic `insertPattern(kind, index)` mints fresh role-named agents (deduped) with reads from the insertion point (`defaultReadsAt`); unreferenced agents GC'd on remove/retarget (`src/lib/nodeRoles.ts`); `uiStore` holds view-only state (view, prompt-book tab, drag, prov hover, rehearsal sampleN) |
