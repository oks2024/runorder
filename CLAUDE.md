...
Test suite: **211 passing** (schema, store, validation, memory names, both emitter snapshots + emit-lines provenance, rehearse derivation, persistence/library/file-IO incl. the `prewire.*`→`playsheet.*`→`runorder.*` storage-key migration, RTL UI incl. rehearsal + prompt-book hover + library menu). `npm run typecheck` / `build` / `lint` clean.
