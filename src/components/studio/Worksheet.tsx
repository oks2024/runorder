 ...
 * masthead, the caps lede, one `PhaseSection` per root step, and a closing colophon. A
 * non-sequence root or a nested-sequence step renders a "flatten it" notice rather than
 * crashing — the same honesty as the emitter, which throws on those.
 */
        ...
        {!isSequence ? (
          ...
          steps.map((node, i) =>
            node.type === 'sequence' ? (
              <p
                key={i}
                className="mt-5 rounded-lg border border-danger/40 bg-danger/5 px-4 py-3 text-[13px] text-danger"
              >
                Phase {i + 1} is a nested sequence — flatten it into top-level phases to edit it
                here.
              </p>
            ) : (
              <PhaseSection
                key={('id' in node && node.id) || i}
